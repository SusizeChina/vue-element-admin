...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.deptNames = selection.map((item) => item.deptName)
    ...
    handleAdd(row) {
      ...
      if (row != undefined) {
        this.form.parentId = row.deptId || this.ids[0]
      }
    ...
    handleDelete(row) {
      ...
      const deptNames = row.deptName || this.deptNames
      this.$confirm('是否确认删除名称为"' + deptNames + '"的数据项?', '警告', {
