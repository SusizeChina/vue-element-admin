      ...
      <el-col :span="20" :xs="24">
        <el-form v-show="showSearch" ref="queryForm" :model="queryParams" :inline="true" label-width="90px">
          <el-form-item label="操作员名称" prop="userName">
            <el-input v-model="queryParams.userName" placeholder="请输入操作员名称" clearable size="small" style="width: 240px" @keyup.enter.native="handleQuery" />
          </el-form-item>
          <el-form-item v-if="!selectUserFlag" label="手机号码" prop="phonenumber">
          ...
          <el-form-item v-if="!selectUserFlag" label="操作员状态" prop="status">
            <el-select v-model="queryParams.status" placeholder="操作员状态" clearable size="small" style="width: 240px">
        ...
        <el-row v-if="!selectUserFlag" :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:user:resetPwd']" type="success" icon="el-icon-refresh" size="mini" :disabled="single" @click="handleResetPwd">重置密码</el-button>
          </el-col>
          <!--
          <el-col :span="1.5">
        ...
        <el-table ref="userTable" v-loading="loading" :data="userList" @selection-change="handleSelectionChange" @select="handleSelect" @select-all="handleSelectAll">
          <el-table-column type="selection" width="50" align="center" />
          <el-table-column label="操作员编号" align="center" prop="userId" :formatter="selectedFormat" />
          <el-table-column label="操作员名称" align="center" prop="userName" :show-overflow-tooltip="true" />
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="操作员名称" prop="nickName">
              <el-input v-model="form.nickName" placeholder="请输操作员名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    selectedFormat(row, column) {
      // 设置默认选中
      const ids = this.selectedUserList.map((item) => item.userId)
    ...
    handleSelect(selection, row) {
      ...
      const ids = selection.map((item) => item.userId)
      const list = this.userList
      for (var i = 0; i < this.userList.length; i++) {
        const data = this.userList[i]
        if (ids.indexOf(data.userId) == -1) { // 没勾选
